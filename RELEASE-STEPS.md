...
App ID `6806789668` / バンドルID `com.zzzjjj080.Mochimono` / **1.1 公開済み。いま `1.2 (4)`（12言語）を提出中。`1.3 (5)`（読み上げ）はビルドを上げ済みで、1.2 の公開待ち**
...
7. ビルド 1.3 (5) を紐づけ → 8. 13言語×2サイズの画像を確かめて提出

課金は 1.2 と一緒に多言語版を審査に出したので、1.3 では足さない。
アーカイブは `/tmp/mochi-1.3.xcarchive`（印 `b40 09/22 16:03`）、2026-09-22 16:04 にアップロード済み。
