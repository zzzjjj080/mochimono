...
final class MochimonoUITests: XCTestCase {
    ...
    func testタップで持った印が付く() {
        let app = launch()
        app.buttons["list-野球"].tap()
        XCTAssertTrue(app.staticTexts["0/11"].waitForExistence(timeout: 5))

        app.buttons["item-帽子"].tap()
        XCTAssertTrue(app.staticTexts["1/11"].waitForExistence(timeout: 3))

        app.buttons["item-帽子"].tap()      // もう一度で外れる
        XCTAssertTrue(app.staticTexts["0/11"].waitForExistence(timeout: 3))
    }
    ...
    func test全部外すは確認を通る() {
        ...
        app.buttons["list-野球"].tap()
        app.buttons["item-帽子"].tap()
        app.buttons["item-バット"].tap()
        XCTAssertTrue(app.staticTexts["2/11"].waitForExistence(timeout: 3))
        ...
        XCTAssertTrue(app.staticTexts["2/11"].waitForExistence(timeout: 3))   // 消えていない
    ...
    func test編集してもチェックが残る() {
        ...
        XCTAssertTrue(app.staticTexts["1/12"].waitForExistence(timeout: 3))
