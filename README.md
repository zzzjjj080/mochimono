...
- **全部そろったら「全部外す」を緑の塗りにして、確認なしのワンタップで外す。**
  そろった盤面で押すべき一手はこれしかないので、毎回止めない。
  代わりに**6秒だけ「元に戻す」の帯**を出す（残り時間はリングの減り方で見せる。盤面タスクと同じ考え方）。
  戻す控え（`PackedSnapshot`）は**保存に入れない**。開き直してまで戻せる必要はない。
